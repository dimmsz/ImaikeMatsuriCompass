...
function layoutVenueLabels() {
  ...
  venueMarkers.forEach(entry => {
    ...
    const point = map.latLngToContainerPoint(entry.marker.getLatLng()); const size = entry.labelSize; const candidates = labelCandidates(point, size); let selected = candidates[0]; let selectedRect = null;
    for (const candidate of candidates) {
      const rect = { left: candidate.x - size.width / 2, right: candidate.x + size.width / 2, top: candidate.y - size.height / 2, bottom: candidate.y + size.height / 2 }; const inside = rect.right >= 0 && rect.left <= map.getSize().x && rect.bottom >= 0 && rect.top <= map.getSize().y;
      if (inside && !occupied.some(other => rectanglesOverlap(rect, other))) { selected = candidate; selectedRect = rect; break; }
      if (!selectedRect) selectedRect = rect;
    }
    occupied.push(selectedRect); entry.labelMarker.setLatLng(map.containerPointToLatLng(selected));
  });
}
...
function matchingSchedules(venue, tokens) {
  const items = venueSchedules(venue.id); if (!tokens.length) return items;
  return items.filter(item => { const searchable = [item.title, item.description, item.genre, ...(Array.isArray(item.tags) ? item.tags : [])].filter(Boolean).join(' ').toLocaleLowerCase(); return tokens.every(token => searchable.includes(token)); });
}
function searchMatchesVenue(venue, tokens) {
  if (!tokens.length) return true;
  const venueText = `${venue.name} ${venue.landmark}`.toLocaleLowerCase();
  return tokens.every(token => venueText.includes(token)) || matchingSchedules(venue, tokens).length > 0;
}
function renderList(items, tokens) {
  venueCount.textContent = `${items.length}会場`;
  venueList.innerHTML = items.length ? items.map(venue => { const matched = matchingSchedules(venue, tokens).length; const total = venueSchedules(venue.id).length; const scheduleText = tokens.length && matched < total ? `検索一致 ${matched}件 / ${total}件` : `タイムスケジュール ${total}件`; return `<button class="venue-card" type="button" data-venue-id="${venue.id}"><h3>${venue.venueNo}. ${escapeHtml(venue.name)}</h3><p>${escapeHtml(venue.landmark)}</p><span class="tag">${scheduleText}</span></button>`; }).join('') : '<p>該当する会場・タイムスケジュールがありません。</p>';
...
async function loadOsmRoads() {
  if (!venues.length) return; const latitudes = venues.map(v => v.lat).filter(Number.isFinite); const longitudes = venues.map(v => v.lng).filter(Number.isFinite); if (!latitudes.length || !longitudes.length) return;
  const paddingLat = 0.001; const paddingLng = 0.001; const bbox = `${Math.min(...latitudes) - paddingLat},${Math.min(...longitudes) - paddingLng},${Math.max(...latitudes) + paddingLat},${Math.max(...longitudes) + paddingLng}`; const query = `[out:json][timeout:25];way["highway"](${bbox});out tags geom;`;
